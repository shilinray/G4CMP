...
//---------------------------------------------------------------------------------------
// Helpers for batch-running analyses over many file pairs in the same folder.
// We assume filenames like:
//   Primary_<TAG>.txt
//   Hits_<TAG>.txt
// where <TAG> encodes the parameter combination (e.g. Al100_Nb20).
static std::vector<std::pair<std::string,std::string>> FindPrimaryHitPairsInDir(
    const std::string& directory,
    const std::string& primaryPrefix = "Primary_",
    const std::string& hitPrefix = "Hits_",
    const std::string& ext = ".txt")
{
  ...
  // Collect tags from Primary_*.txt
  for (const auto& entry : fs::directory_iterator(dirPath)) {
    if (!entry.is_regular_file()) continue;
    const std::string fname = entry.path().filename().string();
    if (fname.rfind(primaryPrefix, 0) != 0) continue; // doesn't start with prefix
    if (entry.path().extension().string() != ext) continue;
    const std::string stem = entry.path().stem().string(); // without .txt
    if (stem.size() <= primaryPrefix.size()) continue;
    const std::string tag = stem.substr(primaryPrefix.size());
    tags.push_back(tag);
  }
  ...
  // Build pairs where both files exist
  for (const auto& tag : tags) {
    fs::path pPrimary = dirPath / (primaryPrefix + tag + ext);
    fs::path pHits    = dirPath / (hitPrefix + tag + ext);
    if (!fs::exists(pHits)) {
      std::cerr << "WARNING: found primary but missing hits for tag '" << tag
                << "' (expected " << pHits.string() << ")\n";
      continue;
    }
    pairs.emplace_back(pPrimary.string(), pHits.string());
  }
  return pairs;
}

static std::string ExtractTagFromFilename(const std::string& pathStr,
                                          const std::string& primaryPrefix = "Primary_")
{
  namespace fs = std::filesystem;
  const std::string stem = fs::path(pathStr).stem().string();
  if (stem.rfind(primaryPrefix, 0) == 0 && stem.size() > primaryPrefix.size()) {
    return stem.substr(primaryPrefix.size());
  }
  return stem;
}
...
//---------------------------------------------------------------------------------------
// Batch version: automatically finds Primary_*.txt and Hits_*.txt pairs in a folder.
// Prints a clean, one-line-per-parameter table.
void PrintPhononCollectionEfficiency(const std::string& directory)
{
  const auto pairs = FindPrimaryHitPairsInDir(directory);
  if (pairs.empty()) {
    std::cerr << "No Primary_/Hits_ pairs found in: " << directory << "\n";
    return;
  }

  std::cout << "\n=== Batch Phonon Collection Efficiency (PCE) ===\n"
            << "Directory: " << directory << "\n"
            << "Found " << pairs.size() << " parameter combinations (" << (2*pairs.size())
            << " files).\n";
  if (pairs.size() != 30) {
    std::cout << "NOTE: You said you expect 30 combinations; I found " << pairs.size() << ".\n";
  }
  ...
  std::cout << "\n"
            ...
            << std::setw(20) << "TAG"
            << std::right
            << std::setw(18) << "E_primary[eV]"
            << std::setw(18) << "E_hits[eV]"
            << std::setw(12) << "PCE"
            << std::setw(12) << "PCE[%]"
            << "\n";
  std::cout << std::string(20+18+18+12+12, '-') << "\n";

  for (const auto& pr : pairs) {
    const std::string& primariesFilename = pr.first;
    const std::string& hitsFilename      = pr.second;
    const std::string tag = ExtractTagFromFilename(primariesFilename);
    ...
    std::cout << std::left
              << std::setw(20) << tag
              << std::right
              << std::setw(18) << std::setprecision(6) << std::fixed << totalPrimaryEnergy_eV
              << std::setw(18) << std::setprecision(6) << std::fixed << totalHitEnergy_eV
              << std::setw(12) << std::setprecision(6) << std::fixed << pce
              << std::setw(12) << std::setprecision(3) << std::fixed << (100.0 * pce)
              << "\n";
  }
...
//---------------------------------------------------------------------------------------
// Batch version of PCEStudy: for each parameter combination, write a *separate* ROOT file
// and label every histogram title with the tag so plots are unambiguous.
void PCEStudy(const std::string& directory)
{
  const auto pairs = FindPrimaryHitPairsInDir(directory);
  if (pairs.empty()) {
    std::cerr << "No Primary_/Hits_ pairs found in: " << directory << "\n";
  ...
  for (const auto& pr : pairs) {
    ...
    // Read in the text files
    std::vector<Event> eventList = ReadInG4CMPPrimaryAndHitFiles(hitsFilename,primariesFilename);
    std::cout << "[" << tag << "] Done reading in " << eventList.size() << " events." << std::endl;

    // Output ROOT file per combination
    const std::string outName = "AnalysisOutput_" + tag + ".root";
    TFile * fOut = new TFile(outName.c_str(),"RECREATE");

    auto titleWithTag = [&](const std::string& base){ return base + " (" + tag + ")"; };

    // Define a number of histograms for the hits
    TH1F * h_nHits = new TH1F("h_nHits", titleWithTag("Number of Hits Per Event; nHits; nEvents").c_str(),100,0,100);
    TH1F * h_eDep  = new TH1F("h_eDep",  titleWithTag("Hit EDeps; log10(eDep[eV]); nEvents").c_str(),200,-6,1);
    TH2F * h_hitXY = new TH2F("h_nHitsXY",titleWithTag("XY Locations of Hits; X [mm]; Y [mm]; NHits/bin").c_str(),200,-5,5,200,-5,5);
    TH2F * h_hitYZ = new TH2F("h_nHitsYZ",titleWithTag("YZ Locations of Hits; Y [mm]; Z [mm]; NHits/bin").c_str(),200,-5,5,200,4.6,5.02);
    TH2F * h_hitXZ = new TH2F("h_nHitsXZ",titleWithTag("XZ Locations of Hits; X [mm]; Z [mm]; NHits/bin").c_str(),200,-5,5,200,4.6,5.02);

    // Define a number of histograms for the primaries
    TH2F * h_primXY = new TH2F("h_primXY",titleWithTag("XY Location of Primary; X [mm]; Y [mm]; Primaries/bin").c_str(),200,-5,5,200,-5,5);
    TH2F * h_primYZ = new TH2F("h_primYZ",titleWithTag("YZ Location of Primary; Y [mm]; Z [mm]; Primaries/bin").c_str(),200,-5,5,200,4.6,5.02);
    TH2F * h_primXZ = new TH2F("h_primXZ",titleWithTag("XZ Location of Primary; X [mm]; Z [mm]; Primaries/bin").c_str(),200,-5,5,200,4.6,5.02);

    // Define histograms useful for calculating PCE
    int nPCEBinsX = 50;
    int nPCEBinsY = 50;
    TH2F * h_totalHitEnergyAtPrimaryXY = new TH2F(
        "h_totalHitEnergyAtPrimaryXY",
        titleWithTag("Total Energy of Hits Generated by Primaries at this XY; X [mm]; Y [mm]; Energy/bin [eV]").c_str(),
        nPCEBinsX,-5,5,nPCEBinsY,-5,5);
    TH2F * h_totalPrimaryEnergyAtPrimaryXY = new TH2F(
        "h_totalPrimaryEnergyAtPrimaryXY",
        titleWithTag("Total Energy of Primaries at this XY; X [mm]; Y [mm]; Energy/bin [eV]").c_str(),
        nPCEBinsX,-5,5,nPCEBinsY,-5,5);
    TH2F * h_pceVsXY = new TH2F(
        "h_pceVsXY",
        titleWithTag("Phonon Collection Efficiency vs. Primary XY; X [mm]; Y [mm]; PCE").c_str(),
        nPCEBinsX,-5,5,nPCEBinsY,-5,5);

    // Loop over events
    for( int iE = 0; iE < (int)eventList.size(); ++iE ){
      if( iE % 1000 == 0 ) std::cout << "[" << tag << "] Done with " << iE << " event histogram fills." << std::endl;
      ...
      h_nHits->Fill(tE.hitVect.size());
      for ( int iH = 0; iH < (int)tE.hitVect.size(); ++iH ){
        const double hitX = tE.hitVect[iH].endX_mm;
        const double hitY = tE.hitVect[iH].endY_mm;
        const double hitZ = tE.hitVect[iH].endZ_mm;
        const double logEnergy_eV = TMath::Log10(tE.hitVect[iH].eDep_eV);
    ...
    for( int iBX = 1; iBX <= h_totalHitEnergyAtPrimaryXY->GetNbinsX(); ++iBX ){
      for( int iBY = 1; iBY <= h_totalHitEnergyAtPrimaryXY->GetNbinsY(); ++iBY ){
        const double num = h_totalHitEnergyAtPrimaryXY->GetBinContent(iBX,iBY);
        const double denom = h_totalPrimaryEnergyAtPrimaryXY->GetBinContent(iBX,iBY);
        double pce = 0.0;
        if( denom != 0.0 ) pce = num/denom;
        h_pceVsXY->SetBinContent(iBX,iBY,pce);
      }
    }
    ...
    fOut->Close();
    delete fOut;
    std::cout << "[" << tag << "] Wrote " << outName << std::endl;
  }
}
