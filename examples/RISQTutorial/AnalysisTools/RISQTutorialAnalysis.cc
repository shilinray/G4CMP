...
void PrintPhononCollectionEfficiencyAndPlot()
{
  ...
  for (int iA = 0; iA < (int)al_vals.size(); ++iA) {
    for (int iN = 0; iN < (int)nb_vals.size(); ++iN) {
      ...
      // Ensure that files exist physically before attempting to parse them!
      std::ifstream infile(primariesFilename.c_str());
      if(!infile.good()) {
          continue; // Skips non-existing outputs generated by your bash script crash bounds.
      }
      infile.close();
