...
struct ParamCombo {
  std::string tag;      // e.g. "Al100_Nb20"
  std::string p1_name;  // e.g. "Al"
  std::string p2_name;  // e.g. "Nb"
  std::string p1_label; // e.g. "100" (kept as string for axis bin labels)
  std::string p2_label; // e.g. "20"
  double pce = 0.0;     // fraction in [0,1]
  double e_primary = 0.0;
  double e_hits = 0.0;
};

static inline void SplitUnderscore(const std::string& s, std::vector<std::string>& out) {
  out.clear();
...
// Parse something like "Al100" into ("Al", "100").
static inline std::pair<std::string,std::string> SplitNameValue(const std::string& token) {
  // Find the first character that can start a number.
  size_t i = 0;
  for (; i < token.size(); ++i) {
  ...
  std::string val  = (i < token.size()) ? token.substr(i) : std::string("");
  // If there wasn't an obvious numeric part, fall back to the whole token as label.
  if (val.empty()) {
    name = token;
    val  = token;
  }
...
// Compute (E_primary, E_hits, PCE) from a Primary + Hits file pair.
static inline ParamCombo ComputePCEForPair(const std::string& primariesFilename,
                                          const std::string& hitsFilename,
  ...
  out.e_primary = totalPrimaryEnergy_eV;
  out.e_hits = totalHitEnergy_eV;
  out.pce = (totalPrimaryEnergy_eV > 0.0) ? (totalHitEnergy_eV / totalPrimaryEnergy_eV) : 0.0;
  return out;
}
...
// {
//   ParamCombo combo = ComputePCEForPair(primariesFilename, hitsFilename, "(single)");

//   std::cout << "\n=== Phonon Collection Efficiency (PCE) ===\n"
//             << "Primaries file: " << primariesFilename << "\n"
//             << "Hits file:      " << hitsFilename << "\n"
//             << "Total primary phonon energy: " << combo.e_primary << " eV\n"
//             << "Total hit phonon energy:     " << combo.e_hits << " eV\n"
//             << "PCE = " << (100.0 * combo.pce) << " %\n"
//             << "========================================\n";
// }

// Batch mode:
//   PrintPhononCollectionEfficiency("/path/to/folder");
//
// Expects files in the folder like:
//   Primary_<TAG>.txt
//   Hits_<TAG>.txt
// where <TAG> contains two underscore-separated parameter tokens, e.g. "Al100_Nb20".
//
// Produces:
//  1) A clean printed table of PCE for each tag.
//  2) A ROOT file PCE_Summary.root containing a TH2D where:
//       x-axis = parameter 1 values
//       y-axis = parameter 2 values
//       z/color = PCE (fraction)
void PrintPhononCollectionEfficiency(std::string directory)
{
  namespace fs = std::filesystem;

  std::map<std::string,std::string> primByTag;
  std::map<std::string,std::string> hitsByTag;

  // Scan folder for Primary_*.txt and Hits_*.txt
  for (const auto& ent : fs::directory_iterator(directory)) {
    if (!ent.is_regular_file()) continue;
    const std::string fn = ent.path().filename().string();
    const std::string full = ent.path().string();

    auto isTxt = [&](const std::string& s){
      return (s.size() >= 4 && s.substr(s.size()-4) == ".txt");
    };
    if (!isTxt(fn)) continue;

    const std::string pfxP = "Primary_";
    const std::string pfxH = "Hits_";
    if (fn.rfind(pfxP, 0) == 0) {
      const std::string tag = fn.substr(pfxP.size(), fn.size() - pfxP.size() - 4);
      primByTag[tag] = full;
    } else if (fn.rfind(pfxH, 0) == 0) {
      const std::string tag = fn.substr(pfxH.size(), fn.size() - pfxH.size() - 4);
      hitsByTag[tag] = full;
    }
  }

  // Compute PCE for each matched tag
  std::vector<ParamCombo> combos;
  combos.reserve(std::min(primByTag.size(), hitsByTag.size()));

  for (const auto& kv : primByTag) {
    const std::string& tag = kv.first;
    auto itH = hitsByTag.find(tag);
    if (itH == hitsByTag.end()) continue;

    ParamCombo c = ComputePCEForPair(kv.second, itH->second, tag);

    // Parse first two underscore-separated tokens into parameter axes.
    std::vector<std::string> toks;
    SplitUnderscore(tag, toks);
    if (toks.size() >= 2) {
      auto nv1 = SplitNameValue(toks[0]);
      auto nv2 = SplitNameValue(toks[1]);
      c.p1_name = nv1.first;
      c.p1_label = nv1.second;
      c.p2_name = nv2.first;
      c.p2_label = nv2.second;
    } else {
      // Fallback: treat whole tag as a single label
      c.p1_name = "param1";
      c.p2_name = "param2";
      c.p1_label = tag;
      c.p2_label = "";
  ...
  if (combos.empty()) {
    std::cerr << "[PrintPhononCollectionEfficiency] No matched Primary_<TAG>.txt / Hits_<TAG>.txt pairs found in: "
              << directory << std::endl;
    return;
  }

  // Sort for a stable printout
  std::sort(combos.begin(), combos.end(), [](const ParamCombo& a, const ParamCombo& b){
    return a.tag < b.tag;
  });

  // Print a clean table
  std::cout << "\n=== Phonon Collection Efficiency (Batch) ===\n";
  std::cout << "Folder: " << directory << "\n\n";
  std::cout << std::left
            << std::setw(18) << "TAG"
            << std::setw(14) << "E_primary[eV]"
            << std::setw(14) << "E_hits[eV]"
            << std::setw(10) << "PCE"
            << std::setw(10) << "PCE[%]"
            << "\n";
  std::cout << std::string(18+14+14+10+10, '-') << "\n";
  for (const auto& c : combos) {
    std::cout << std::left
              << std::setw(18) << c.tag
              << std::setw(14) << c.e_primary
              << std::setw(14) << c.e_hits
              << std::setw(10) << c.pce
              << std::setw(10) << (100.0*c.pce)
              << "\n";
  }
  std::cout << "===========================================\n";

  // Build labeled-bin axes for TH2D
  std::string p1name = combos[0].p1_name;
  std::string p2name = combos[0].p2_name;

  std::vector<std::string> xLabels;
  std::vector<std::string> yLabels;
  {
    std::map<std::string,int> seenX, seenY;
    for (const auto& c : combos) {
      if (!seenX.count(c.p1_label)) { seenX[c.p1_label] = 1; xLabels.push_back(c.p1_label); }
      if (!seenY.count(c.p2_label)) { seenY[c.p2_label] = 1; yLabels.push_back(c.p2_label); }
    }
    std::sort(xLabels.begin(), xLabels.end());
    std::sort(yLabels.begin(), yLabels.end());
  }

  auto labelToBin = [](const std::vector<std::string>& labels){
    std::map<std::string,int> m;
    for (int i = 0; i < (int)labels.size(); ++i) m[labels[i]] = i+1; // ROOT bins start at 1
    return m;
  };
  const auto xBin = labelToBin(xLabels);
  const auto yBin = labelToBin(yLabels);

  const std::string outRoot = (fs::path(directory) / "PCE_Summary.root").string();
  TFile* fOut = new TFile(outRoot.c_str(), "RECREATE");

  TH2D* h_pce_params = new TH2D(
      "h_pce_params",
      ("PCE vs Parameters;" + p1name + ";" + p2name + ";PCE").c_str(),
      (int)xLabels.size(), 0, (int)xLabels.size(),
      (int)yLabels.size(), 0, (int)yLabels.size());

  // Set bin labels
  for (int i = 0; i < (int)xLabels.size(); ++i) {
    h_pce_params->GetXaxis()->SetBinLabel(i+1, xLabels[i].c_str());
  }
  for (int j = 0; j < (int)yLabels.size(); ++j) {
    h_pce_params->GetYaxis()->SetBinLabel(j+1, yLabels[j].c_str());
  }

  // Fill z=content with PCE
  for (const auto& c : combos) {
    auto ixIt = xBin.find(c.p1_label);
    auto iyIt = yBin.find(c.p2_label);
    if (ixIt == xBin.end() || iyIt == yBin.end()) continue;
    h_pce_params->SetBinContent(ixIt->second, iyIt->second, c.pce);
  }
